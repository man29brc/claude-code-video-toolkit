...
- **WVS standard pipeline (roadmap):** Flux/Ideogram/Imagen stills → Runware image-to-video (pay-per-use; replaces Luma) → Sarvam Bulbul Gujarati voice → DaVinci Resolve → Zoho Social. Rameshbhai is the recurring character: keep his description identical in every prompt. Gujarati/Hindi only, ARN tagline at the start.
